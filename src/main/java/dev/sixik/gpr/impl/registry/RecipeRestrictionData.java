...
public record RecipeRestrictionData(
    ...
    public static int[] packData(int recipeType, int[] recipes) {
        ...
        int[] flatData = new int[count + GROUP_HEADER_SIZE];
    ...
    public static int[] packData(int[] recipes) {
        ...
        for (int recipeId : recipes) {
            RecipeHolder<?> holder = managerExtern.getRecipeHolderByIndex(recipeId);
            RecipeType<?> type = holder.value().getType();
            int typeIndex = RecipeTypeIndex.get(type).gpr$getIndex();
            groupedRecipes.computeIfAbsent(typeIndex, ignored -> new IntArrayList()).add(recipeId);
        }

        int totalSize = (groupedRecipes.size() * GROUP_HEADER_SIZE) + recipes.length;
        int[] flatData = new int[totalSize];
        int[] sortedTypeIndices = groupedRecipes.keySet().toIntArray();
        int pointer = 0;
